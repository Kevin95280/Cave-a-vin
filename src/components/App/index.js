...
import wines from '../../data/data.js';
...
const App = () => {
    ...
    const getCardsFromData = () => {
        ...
        if (!search.trim()) { // Si le champ est vide ou ne contient que des espaces
            setCards(wines); // Affiche toutes les cartes
            return;
        }

        const filteredCards = wines.filter(item =>
        ...
        console.log("Données filtrées :", filteredCards);
